...
class JsMapView extends JsMapObject implements IFMapDispose{
	...
	protected void createObject(JSONArray pJsArgs) {
		int _l = Integer.parseInt(JSONUtil.getString(pJsArgs,0));
		int _t = Integer.parseInt(JSONUtil.getString(pJsArgs,1));
		int _w = Integer.parseInt(JSONUtil.getString(pJsArgs,2));
		int _h = Integer.parseInt(JSONUtil.getString(pJsArgs,3));
		mMapFrameItem.createMap(_l, _t, _w, _h);
	}
	@Override
	public void dispose() {
