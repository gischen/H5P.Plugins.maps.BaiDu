...
#define GETBYTE_A(dw) ((Byte)(((uint)(dw) >> 16) & 0xFF))
#define GETBYTE_B(dw) ((Byte)(((uint)(dw) >> 8) & 0xFF))
#define GETBYTE_C(dw) ((Byte)((uint)(dw) & 0xFF))

//TODO..
UIColor* NSString2UIColor(NSString* color)
{
    NSUInteger len = [color length];
    if ( len > 2 && '#' == [color characterAtIndex:0] )
    {
        ...
        colorValue = (int)strtoul([[color substringWithRange:range ] UTF8String], 0, 16);
        red = GETBYTE_A(colorValue);
        green = GETBYTE_B(colorValue);
        blue = GETBYTE_C(colorValue);;
        return [UIColor colorWithRed:red green:green blue:blue alpha:1.0];
    }/*
    if ( color && '#' == [color characterAtIndex:0] )
    {
        CGFloat red,green,blue;
        NSRange range;
        range.location = 1;
        range.length = 2;
        red = strtoul([[color substringWithRange:range ] UTF8String], 0, 16);
        range.location = 3;
        green = strtoul([[color substringWithRange:range ] UTF8String], 0, 16);
        range.location = 5;
        blue = strtoul([[color substringWithRange:range ] UTF8String], 0, 16);
        return [UIColor colorWithRed:red green:green blue:blue alpha:1.0];
    }*/
    return nil;
}
